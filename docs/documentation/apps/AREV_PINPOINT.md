...
Help players learn where countries, capitals, and cities are by tapping their location on a map.

The closer the tap is to the target, the better the score.
...
- if distance-to-polygon is not ready, use centroid distance temporarily and document it in code/tests.
...
- distance from tap to capital coordinate.
- perfect threshold: suggested <= 25 km.
...
Official presets should be fixed and limited.

### 5. Obscure Places

Uses obscure country/city filters.

Local-only until the data pool is validated.
...
- off.
- continent labels.
- country labels, study/custom only.
...
Tiny countries must not feel impossible.

Use:

- zoom support.
- tap tolerance.
- inset/expanded hit region in some modes.
- special scoring tolerance.
